...
my $DependenciesLinks=Dependencies::InitializeDependencies($ENV{OUTBIN_DIR}, $MakefileName, $SRC_DIR, $OUTPUT_DIR, $CASE_SENSITIVE, Dependencies::CHILDREN );
...
my $iCountFile=0;
my $iP4FilesHavingDelivrables=0;
my $iTotalFoundDelivrableFiles=0;

foreach my $P4HaveEntry(@$raP4FilesList)
{
	next unless(my($DepotFile,$DiskFile) = $P4HaveEntry=~ /^(.+?)(?:#\d*)? - (.*)$/);
	next if(exists $AlreadyAnalyzedClientFiles{$DiskFile});
	my $rhFoundDelivrablesDependenciesForThisFile=Dependencies::FindFinalDelivrableDependencies(++$iCountFile, $DependenciesLinks, $DelivrableFiles, $DiskFile, undef, {'$SRC_DIR'=>"^".$SRC_DIR, '$OUTPUT_DIR'=>"^".$OUTPUT_DIR }, $CASE_SENSITIVE);
	if(defined $rhFoundDelivrablesDependenciesForThisFile)
	{
		$iTotalFoundDelivrableFiles+=keys %$rhFoundDelivrablesDependenciesForThisFile;
		open(DEPENDENCIESRESULT, ">>$OUTPUT_DIR/logs/packages/$MakefileName.delivrables.dat") or die("ERROR: cannot open '$OUTPUT_DIR/logs/packages/$MakefileName.delivrables.dat': $!");
		foreach my $DelivrableFile(keys %$rhFoundDelivrablesDependenciesForThisFile)
		{
			print(DEPENDENCIESRESULT "$DepotFile -> $DelivrableFile\n");
		}
		close(DEPENDENCIESRESULT);
		$iP4FilesHavingDelivrables++;
	}
    open(DEPENDENCIESDONE, ">>$OUTPUT_DIR/logs/packages/$MakefileName.delivrables.done") or die("ERROR: cannot open '$OUTPUT_DIR/logs/packages/$MakefileName.delivrables.done': $!");    
    print(DEPENDENCIESDONE ('$AlreadyAnalyzedClientFiles{'."'".$DiskFile."'"."}=0;\n"));
    close(DEPENDENCIESDONE);
}
...
printf("    Number of found delivrables: ".$iTotalFoundDelivrableFiles."\n");
printf("    Number of P4 files having delivrables: ".$iP4FilesHavingDelivrables."\n");
