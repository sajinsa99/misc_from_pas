...
if ($ENV{PLATFORM} eq "hpux_ia64" || $srcDestModel{$ENV{PLATFORM}} eq "64") {
	if ($ENV{PLATFORM} eq "win32_x86")
	{
		
		die ("ERROR: CONTEXT not set")		unless($ENV{CONTEXT});
		die ("ERROR: BUILD_NUMBER not set")	unless($ENV{BUILD_NUMBER});
		
		my $buildNumber32 = $ENV{BUILD_NUMBER} || $ENV{build_number};
		my $duDestDIR32 = "$ENV{OUTPUT_DIR}/deploymentunits";
		my $duSrcDIR32 = $ENV{DUSRC_DIR} || "$ENV{DROP_DIR}/$ENV{context}/$buildNumber32/$srcDestPlatform{$ENV{PLATFORM}}/$ENV{BUILD_MODE}/deploymentunits";
		$duSrcDIR32 =~ s-\/\/-\\\\-g;

		my $exportfinished32 = "$duSrcDIR32/deploymentunits_copy_done";
    print "Waiting for $exportfinished32\n";
    while ( ! -e "$exportfinished32" ) {
      print ".";
      sleep(10);
    }
    
    system "cp -prvu $duSrcDIR32/setup.engine/ $duDestDIR32";
...
if ($ENV{COPY_DU_SYNC}) {
    ...
    print "Waiting for $exportfinished\n";
...
foreach my $du (@DUs) {
    ...
    unless( -e "$duDestDIR/$dupath") {
		mkpath("$duDestDIR/$dupath") or warn("ERROR: cannot mkpath '$duDestDIR': $!");
		$copyneeded = 1;
    }
